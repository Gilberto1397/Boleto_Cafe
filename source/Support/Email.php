...
class Email
{
    ...
    /**
     * queue
     * agendamento dos disparos de email
     * @param  string $from
     * @param  string $fromName
     * @return bool
     */
    public function queue(string $from = CONF_MAIL_SENDER['address'], string $fromName = CONF_MAIL_SENDER["name"]): bool // configuração do remetente
    {       // armazena os emails a serem enviados no DB
        try {
     ...
     * @param int $perSecond
     */
    public function sendQueue(int $perSecond = 5) //uma mensagem a cada 5seg
    {   // realizará o disparo da fila de emails

        $stmt = Connect::getInstance()->query("SELECT * FROM mail_queue WHERE sent_at IS NULL"); // buscando os emails ainda não foram disparados
        if ($stmt->rowCount()) {
            foreach ($stmt->fetchAll() as $send) {
                $email = $this->bootstrap(  // usando o próprio bootstrap para junto do objeto montar o método de montagem do email
                    $send->subject,
                    $send->body,
                ...
                if ($email->send($send->from_email, $send->from_name)) { // passado parâmetros no send devido poder ser diferentes remetentes
                    usleep(1000000 / $perSecond);
                    Connect::getInstance()->exec("UPDATE mail_queue SET sent_at = NOW() WHERE id = {$send->id}"); // atualização do sent_at do email armazenado para não envia-lo novamente
                }
            }
        }
    }
